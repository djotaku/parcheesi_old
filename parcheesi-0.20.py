                    ...
                    for player in players:
                        ...
                        if player.piece1.inbase:
                            if dice.die1 == 5 or dice.die2 == 5:
                                ...
                                player.piece1.leave_base(board.spaces[(player.startingpos+dice_total)%68])
                                player.piece1.inbase = False
                                board.location[21+dice_total][0]=1
                                player.piece1.space = (player.startingpos + dice_total)%68
                        if not player.piece1.inbase:
                            if (player.piece1.space + dice_total)%68 == 0:
                                player.piece1.update_pos(board.spaces[(player.piece1.space + dice_total+1)%68])
                            else:
                                player.piece1.update_pos(board.spaces[(player.piece1.space + dice_total)%68])
                            player.piece1.space = (player.piece1.space + dice_total)%68
                            board.location = board.location_old
                            board.location[(player1.piece1.space + dice_total-1)%68][0] = 1
                        pygame.display.flip()
            #if e.type == pygame.MOUSEBUTTONDOWN:
