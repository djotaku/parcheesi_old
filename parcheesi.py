...
#TODO
#-when two are on the same space they look correct then implement capturing/blocking
...
class Board():
    ...
    def exitbase(self, player, piece, dice_total):
        """This will handle getting the pieces out of teh base"""
        player.piecesinbase = player.piecesinbase - 1
        piece.leave_base(self.spaces[(player.startingpos+dice_total)%68])
        piece.inbase = False
        self.location[(player.startingpos-1)+dice_total][0]=1
        piece.space = (player.startingpos + dice_total)%68
        #for debugging purposes
        print player.playername + "'s space:", piece.space

    def movepiece(self, player, piece, dice_total):
        """This will handle moving the pieces around the board"""
        if (piece.space + dice_total)%68 == 0:
            piece.update_pos(self.spaces[(piece.space + dice_total+1)%68])
        else:
            piece.update_pos(self.spaces[(piece.space + dice_total)%68])
        self.location[(piece.space + dice_total-1)%68][0] = 0
        piece.space = (piece.space + dice_total)%68
            #for debugging purposes
        print player.playername + "'s space:", piece.space
        self.location[(piece.space + dice_total-1)%68][0] = 1
...
def main():
    ...
    while done == False:
        ...
        if dice.rolled == True:
            ...
            if players[playerindex].piecesinbase == 4:
                if dice.die1 == 5 or dice.die2 == 5:
                    board.exitbase(players[playerindex], players[playerindex].piece1, dice_total)
                    players[playerindex].piece1.left = True
            else:
                if dice.die1 == 5 or dice.die2 == 5:
                    print "we'd give the chance to have another piece exit, but for now the game will proceed as before"
                    board.movepiece(players[playerindex], players[playerindex].piece1, dice_total)
                    pygame.display.flip()
                else:
                    board.movepiece(players[playerindex], players[playerindex].piece1, dice_total)
                    pygame.display.flip()
